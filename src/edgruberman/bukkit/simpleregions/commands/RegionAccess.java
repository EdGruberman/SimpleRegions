    ...
    private static void edit(final Context context, final edgruberman.bukkit.simpleregions.Region region, final List<String> names, final boolean grant) {
        ...
        for (final String name : names) {
            if (!(grant ? region.access.grant(name) : region.access.revoke(name))) {
                Main.messageManager.tell(context.sender, "Region access " + (grant ? "already contains " : "does not contain ") + name + " in " + region.getDisplayName(), MessageLevel.WARNING, false);
                continue;
            }

            Main.messageManager.tell(context.sender, "Region access " + (grant ? "granted to " : "revoked from ") + name + " for " + region.getDisplayName(), MessageLevel.STATUS, false);
            if (region.isActive())

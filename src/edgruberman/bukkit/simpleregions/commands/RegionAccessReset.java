...
public class RegionAccessReset implements CommandExecutor {
    ...
    public RegionAccessReset(final Plugin plugin, final Catalog catalog) {
        this.plugin = plugin;
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        MessageManager.of(this.plugin).tell(sender, String.format("Region §2access reset§r to %1$s for %2$s in %3$s", access, region.getDisplayName(), region.world.getName()), MessageLevel.STATUS, false);
        return true;
    }

...
public class RegionOwnersRevoke implements CommandExecutor {
    ...
    public RegionOwnersRevoke(final Plugin plugin, final Catalog catalog) {
        this.plugin = plugin;
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (!region.isDirectOwner(owner)) {
            MessageManager.of(this.plugin).tell(sender, String.format("Region owners §edoes not contain§r %1$s", owner), MessageLevel.WARNING, false);
            return true;
        }
        ...
        MessageManager.of(this.plugin).tell(sender, String.format("Region §2ownership removed§r from %1$s for %2$s in %3$s", owner, region.getDisplayName(), region.world.getName()), MessageLevel.STATUS, false);
        return true;
    }

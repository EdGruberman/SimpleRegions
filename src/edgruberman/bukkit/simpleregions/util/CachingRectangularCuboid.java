...
public class CachingRectangularCuboid {
    ...
    protected static final int DEFAULT_COORDINATE_FORMAT = 3;
    ...
    public String describeCoordinates(final Integer format) {
        ...
        final int longestMin = Math.max(lengthMinX, Math.max(lengthMinY, lengthMinZ));
        ...
        final int longestMax = Math.max(lengthMaxX, Math.max(lengthMaxY, lengthMaxZ));

        final int longestX = Math.max(lengthMinX, lengthMaxX);
        final int longestY = Math.max(lengthMinY, lengthMaxY);
     // int longestZ = Math.max(lengthMinZ, lengthMaxZ);
        ...
        final String xS = (xN.equals("x1") ? "x2" : "x1");
        ...
        final String yU = (yD.equals("y1") ? "y2" : "y1");
        ...
        final String zW = (zE.equals("z1") ? "z2" : "z1");
        ...
        switch ((format != null ? format : CachingRectangularCuboid.DEFAULT_COORDINATE_FORMAT)) {
            ...
            case 2:
                ...
                //          x1: -1500  [S]             y1:   0 [D]
                return String.format(
                    CachingRectangularCuboid.repeat(" ", 5 + lengthMaxZ)
                    + "%1$s: %2$#" + longestX + "s  [N]"
                    + CachingRectangularCuboid.repeat(" ", 10 + lengthMinZ) + "%3$s: %4$#" + longestY + "s [U]" + "\n"
                    + "%5$s: %6$#s" + CachingRectangularCuboid.repeat(" ", 5 + longestX - 1)
                    + "[W]   [E]  %7$s: %8$#s" + CachingRectangularCuboid.repeat(" ", 9 + longestY) + "\n"
                    + CachingRectangularCuboid.repeat(" ", 5 + lengthMaxZ) + "%9$s: %10$#" + longestX + "s  [S]"
                    + CachingRectangularCuboid.repeat(" ", 10 + lengthMinZ) + "%11$s: %12$#" + longestY + "s [D]"
